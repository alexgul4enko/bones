...
function HooksCheck(){
  return (
    <>
      ...
      <p data-testid="F_PROTECTED+ROLE_A-hook">F_PROTECTED + ROLE_A: {useHasAccess([F_PROTECTED, 'ROLE_A' ]) ? 'TRUE' : 'FALSE'}</p>
      <p data-testid="F_UNAUTHORISED+ROLE_A-hook">F_UNAUTHORISED + ROLE_A: {useHasAccess([F_UNAUTHORISED, 'ROLE_A' ]) ? 'TRUE' : 'FALSE'}</p>
      <p data-testid="F_UNAUTHORISED||ROLE_A-hook">F_UNAUTHORISED || ROLE_A: {useHasAccess([F_UNAUTHORISED, 'ROLE_A' ], 'SOME') ? 'TRUE' : 'FALSE'}</p>
...
describe(specTitle('@cranium/access'), () => {
  it('should render components bases on permissions', () => {
    ...
    cy.mount(
      ...
      >
        ...
        <CheckAccess access={["ROLE_A", F_PROTECTED]} fallback={<p data-testid="ROLE_A+F_PROTECTED-JSX">fallback: ROLE_A+F_PROTECTED</p>}>
          <p data-testid="ROLE_A+F_PROTECTED-JSX">access: ROLE_A + F_PROTECTED</p>
        </CheckAccess>
        <CheckAccess access={["ROLE_A", F_PROTECTED, F_ADMIN]} fallback={<p data-testid="ROLE_A+F_PROTECTED+F_ADMIN-JSX">fallback: ROLE_A+F_PROTECTED+F_ADMIN</p>}>
          <p data-testid="ROLE_A+F_PROTECTED+F_ADMIN-JSX">access: ROLE_A + F_PROTECTED + F_ADMIN</p>
        </CheckAccess>
        <CheckAccess access={["ROLE_A", F_PROTECTED, F_ADMIN]} operator="SOME" fallback={<p data-testid="ROLE_A||F_PROTECTED||F_ADMIN-JSX">fallback: ROLE_A || F_PROTECTED || F_ADMIN</p>}>
          <p data-testid="ROLE_A||F_PROTECTED||F_ADMIN-JSX">access: ROLE_A || F_PROTECTED || F_ADMIN</p>
        </CheckAccess>
        <CheckAccess access={["ROLE_C", F_SUPER_USER, F_ADMIN]} operator="SOME" fallback={<p data-testid="ROLE_C||F_SUPER_USER||F_ADMIN-JSX">fallback: ROLE_C || F_SUPER_USER || F_ADMIN</p>}>
    ...
    cy.get('[data-testid="ROLE_A+F_PROTECTED+F_ADMIN-JSX"]').should('not.contain', 'access: ROLE_A + F_PROTECTED + F_ADMIN');

    cy.get('[data-testid="ROLE_A||F_PROTECTED||F_ADMIN-JSX"]').should('contain', 'access: ROLE_A || F_PROTECTED || F_ADMIN');
    cy.get('[data-testid="ROLE_A||F_PROTECTED||F_ADMIN-JSX"]').should('not.contain', 'fallback: ROLE_A || F_PROTECTED || F_ADMIN');

    cy.get('[data-testid="ROLE_C||F_SUPER_USER||F_ADMIN-JSX"]').should('contain', 'fallback: ROLE_C || F_SUPER_USER || F_ADMIN');
    cy.get('[data-testid="ROLE_C||F_SUPER_USER||F_ADMIN-JSX"]').should('not.contain', 'access: ROLE_C || F_SUPER_USER || F_ADMIN');
  ...
  it('should pass all props to acessLevels', () => {
    ...
    const acessLevels =  composeAccess(
      //if there is user id then user is authorized and has access to portal.
      (props) => Object.values(props).reduce((res, prop)=>{
    ...
    cy.get('[data-testid="all-permission"]').should('contain', 'ROLE_A, ROLE_B, PERMISSION_A, PERMISSION_B, F_UNAUTHORISED');
  })
})