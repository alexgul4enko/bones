...
function AllPermissions() {
  ...
  return <p data-testid='all-permission'>{Array.from(permissions).join(', ')}</p>;
}
...
function HooksCheck() {
  return (
    <>
      ...
      <p data-testid='empty-hook'>empty: {useHasAccess(undefined) ? 'TRUE' : 'FALSE'}</p>
...
describe(specTitle('@cranium/access'), () => {
  it('should render components bases on permissions', () => {
    const acessLevels = composeAccess(
      ...
      (props: { userRoles: string[] }) => props.userRoles,
  ...
  it('should pass all props to acessLevels', () => {
    const acessLevels = composeAccess(
      ...
      (props: Record<string, string | string[]>) =>
        Object.values(props).reduce((res: string[], prop: string | string[]) => {
