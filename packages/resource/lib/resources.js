...
export function makeRequest(httpRequest) {
  return function request(payload, meta) {
    return (dispatch, getState, { API }) => {
      let { type, endpoint, queries = [], forceUpdates } = meta;
      ...
      const wrappedPromise = httpRequest(
        API,
        payload,
        { signal: controller.signal, ...meta, endpoint },
        { dispatch, getState }
      )
        .then((response) => {
          dispatch(
...
export default function connectResources(resource) {
  return connect(mapStateToProps(resource), (dispatch) => mapDispatchToProps(resource, dispatch), mergeProps);
}
